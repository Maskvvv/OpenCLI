...
async function applyInterestedInviteFilter(page, delayBounds) {
    ...
    const clickedFilter = await clickText(page, ['感兴趣']);
    if (!clickedFilter) {
...
async function clickRevealContactButton(page) {
    ...
    () => {
      ...
      const surface = document.querySelector('.ant-modal, .semi-modal, [role="dialog"], [class*="drawer"], [class*="popover"]') || document.body;
      const label = Array.from(surface.querySelectorAll('*'))
        .filter(isVisible)
        .find((el) => /博主微信|联系电话|联系方式/.test(textOf(el)));
      ...
      return candidates.sort((a, b) => a.getBoundingClientRect().left - b.getBoundingClientRect().left)[0] || null;
    }
  `);
}
...
async function readWechatContact(page) {
    ...
    (() => {
      ...
      const nodes = Array.from(document.querySelectorAll('*')).filter((el) => {
        ...
        return rect.width > 0 && rect.height > 0 && (nodeText.includes('博主微信') || nodeText.includes('联系电话'));
      });
      for (const node of nodes) {
        ...
        const idx = siblings.findIndex((value) => value.includes('博主微信') || value.includes('联系电话'));
        if (idx >= 0 && siblings[idx + 1]) return siblings[idx + 1].replace(/^[:：]/, '').trim();
      }
      return '';
    })()
  `);
...
async function collectCurrentPage(page, delayBounds, pageNum) {
    ...
    for (let i = 0; i < inviteRows.length; i++) {
        await humanPause(page, delayBounds, 0.7);
        const invite = inviteRows[i];
        const contact = await readContactInfoByKolId(page, invite.kolId);
        await humanPause(page, delayBounds, 0.5);
